...
export default function DegreePlanner({ user, setUser, degreePlan }) {
  ...
  const [isSaved, setIsSaved] = useState(false); // New state to track if the degree plan has been saved
  ...
  // Filtered courses logic (unchanged)
  ...
  // Save degree plan logic (updated to set isSaved to true)
  const handleSaveDegreePlan = () => {
    ...
    setIsSaved(true); // Set isSaved to true when the degree plan is saved
  };

  // Generate PDF logic (unchanged)
  const handleCreatePDF = () => {
    ...
    semesters.forEach((semester, index) => {
      ...
      doc.text(semesterTitle, 10, 20 + index * 10);
      semester.courses.forEach((course, courseIndex) => {
        const courseText = `${course.name}`;
        doc.text(courseText, 10, 30 + index * 10 + courseIndex * 10);
      });
    });
  ...
  const getErrorMessages = (array) => {
    ...
    if (notEnoughHoursArray.length != 0) {
      ...
      NOT_ENOUGH_HOURS += notEnoughHoursArray.map((err) => err.semester).join(", ");
    ...
    if (prerequisiteConflictArray.length != 0) {
      ...
      PREREQUISITE_CONFLICT += prerequisiteConflictArray.map((err) => {
  ...
  return (
    <div className="grid grid-cols-12 gap-6 w-full h-full min-h-screen bg-white dark:bg-gray-900 py-6 px-20">
      ...
      <div className="col-span-3 space-y-6" style={{ height: "95vh" }}>
